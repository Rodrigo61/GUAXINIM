...
vii get_adj(pii root, int R, int C) {
    ...
    if (VALID(root.first - 1, root.second - 1, R, C)) {
...
int main(){
    ...
    for (size_t q = 0; q < n_cases; q++) {
        ...
        for (size_t i = 0; i < R; i++) {
            ...
            if (pos = map[i].find("S"), pos != string::npos) {
                source = mp(i, pos);
            }

            if (pos = map[i].find("T"), pos != string::npos) {
                target = mp(i, pos);
            }
        ...
        while (!que.empty()) {
            ...
            for (auto &v : get_adj(u, R, C)) {
                if (!visited[v.first][v.second] && map[v.first][v.second] != '#') {
        ...
        deb("dist[target.first][target.second] = ", dist[target.first][target.second]);
        if (dist[target.first][target.second] <= oxyg) {
            cout << "Possible" << endl;
        } else {
