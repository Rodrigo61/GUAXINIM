...
/*
	Init V_left e V_right
	Não usar vértice 0, pois esse será o vértice dummy do algoritmo
*/
...
bool bfs() {
    ...
    // começa um BFS a partir de todo vertice livre (i.e. match[u] == 0) da esquerda
...
bool dfs(int u) {
    ...
    if (u == 0) {
        return true;  // cheguei no dummy, significa que cheguei em alguem na direita sem match
    }
...
int main(){
    ...
    int E;
    cin >> V_left >> V_right >> E;


    vet_adj.resize(V_left + 1);

    for (int i = 0; i < E; ++i) {
        int u, v;
        cin >> u >> v;

        vet_adj[u].pb(v + V_left);
    }
