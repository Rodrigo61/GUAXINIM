...
int main(){
    ...
    for (size_t i = 0; i < N; i++) {
        if (vet[i].first == 0){
            if (vet[i].second + 1 < mini) {
                begin = 0;
                end = vet[i].second;
                found = true;
                deb("new begin zero= ", begin);
                deb("new end zero= ", end);
                break;
            }
        }
    }
