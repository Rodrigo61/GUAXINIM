...
int main(){
    ...
    while (cin >> N) {
        ...
        print_matrix_debug(row_color);


        color = 1;
        used_color = false;
        vector<vi> col_color(N, vi(N));
        for (int j = 0; j < N; ++j) {
            for (int i = 0; i < N; ++i) {
        ...
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < N; ++j) {
                ...
                if (row_color[i][j] != -1) {
                        vet_adj[row_color[i][j]].pb(col_color[i][j] + V_left);
                }
            }
        }
