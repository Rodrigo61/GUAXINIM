...
/*
	...
	Nao usar o vertice 0, pois esse sera o vertice dummy dobool debug = true;
...
int main(){
    ...
    int n_horses, n_soldiers, n_bond;
    int n_cases = 0;

    while (cin >> n_horses >> n_soldiers >> n_bond) {

        vi start_bucket(n_horses + 1);

        vet_adj.clear();
        vet_adj.resize(1000000);
        ...
        for (int i = 0; i < n_horses; ++i) {
            ...
            start_bucket[i] = last_start;

            last_start += capacity;
        }
        start_bucket[n_horses] = last_start;
        ...
        deb("V_left = ", V_left);
        deb("V_right = ", V_right);
        print_vector_debug(start_bucket);
        ...
        for (int i = 0; i < n_bond; ++i) {
            ...
            for (int i = start_bucket[u]; i < start_bucket[u + 1]; ++i) {
                vet_adj[i].pb(v + V_left + 1);
            }
        }
