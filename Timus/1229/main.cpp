        ...
        for (int j = 0; j < C; j++) {
            if (a_turn) {
                if (valid(i - 1, j) && grid[i - 1][j] != grid[i][j]) {
                    vet_adj[map_id[i][j]].pb(map_id[i - 1][j]);
                }
                if (valid(i + 1, j) && grid[i + 1][j] != grid[i][j]) {
                    vet_adj[map_id[i][j]].pb(map_id[i + 1][j]);
                }
                if (valid(i, j - 1) && grid[i][j - 1] != grid[i][j]) {
                    vet_adj[map_id[i][j]].pb(map_id[i][j - 1]);
                }
                if (valid(i, j + 1) && grid[i][j + 1] != grid[i][j]) {
                    vet_adj[map_id[i][j]].pb(map_id[i][j + 1]);
                }
        ...
        for (size_t j = 0; j < C; j++) {
            if (a_turn){
                if (final_grid[i][j] == -1) {
                    ...
                    pii pair = id2coord[match[map_id[i][j]]];
